  ...
  <section class="py-16 px-4 bg-gray-50">
    <div class="max-w-7xl mx-auto" v-if="step === 1">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-lg text-gray-600 max-w-3xl mx-auto">
          Choisissez le type de QR code qui correspond à vos besoins parmi notre
          large sélection
        </p>
      </div>
          ...
          <div class="relative">
            ...
            <div class="relative bg-gray-900 rounded-[2.5rem] p-2 shadow-2xl">
              <div class="bg-black rounded-[2rem] p-1">
                <div
                  class="bg-white rounded-[1.5rem] overflow-hidden"
                  style="width: 280px; height: 580px"
                >
                  <!-- Status bar -->
                  <div
                    class="bg-white px-6 py-2 flex justify-between items-center"
                  >
                    ...
                    <div class="flex items-center space-x-1">
                      ...
                      <svg class="w-4 h-4" fill="black" viewBox="0 0 24 24">
                        <path
                          d="M2 17h20v2H2zm1.15-4.05L4 11l.85 1.95L7 13l-1.15.05zm3.82-2.13l1.38-1.38c.09.46.27.91.54 1.3l-1.3 1.3c-.4-.27-.84-.45-1.3-.54z"
                        />
                      </svg>
                    </div>
                  </div>
                  ...
                  <div class="px-6 py-4 h-full bg-gray-50">
                    <div class="text-center mb-6">
                      <h3 class="text-lg font-bold text-gray-900 mb-2">
                        {{ displayedType ? 'Aperçu du QR Code' : 'Sélectionnez un type de code QR sur la gauche' }}
                      </h3>
                    </div>
                    ...
                    <div class="flex justify-center mb-6">
                      <div
                        class="bg-white p-4 rounded-2xl shadow-lg border-2 border-gray-100"
                      >
                        <div
                          class="w-32 h-32 bg-white rounded-lg flex items-center justify-center"
                        >
                          <div v-if="displayedType" class="text-center">
                            <!-- QR Code simulé -->
                            <div
                              class="w-28 h-28 bg-black rounded-lg relative overflow-hidden"
                            >
                              ...
                              <div class="absolute inset-2 bg-white rounded-sm">
                                ...
                                >
                                  <div
                                    class="w-8 h-8 bg-white rounded-full border-2 border-gray-200 flex items-center justify-center"
                                  >
                                    <div
                                      v-if="displayedType.icon.includes('svg')"
                                      v-html="displayedType.icon"
                                      class="w-4 h-4 text-blue-500"
                                    ></div>
                                    <span
                                      v-else
                                      class="text-blue-500 text-xs"
                                      >{{ displayedType.icon }}</span
                                    >
                                  </div>
                                </div>
                              </div>
                          ...
                          <div v-else class="text-gray-400 text-center">
                            ...
                            >
                              <path
                                ...
                                stroke-linejoin="round"
                                stroke-width="1"
                                d="M4 4v6h6V4H4zm10 0v6h6V4h-6zM4 14v6h6v-6H4zm10 0v6h6v-6h-6z"
                              />
                            </svg>
                    ...
                    <!-- Informations sur le type affiché -->
                    <div
                      v-if="displayedType"
                    ...
                    >
                      <div class="flex items-center space-x-3 mb-3">
                        <div
                          :class="[
                            ...
                            hoveredType ? 'bg-gray-500' : 'bg-blue-500'
                          ]"
                        >
                          <div
                            v-if="displayedType.icon.includes('svg')"
                      ...
                      <button
                        v-else-if="hoveredType"
                        @click="selectQRType(hoveredType)"
                        class="w-full bg-gray-500 text-white py-2.5 px-4 rounded-lg font-medium text-sm hover:bg-gray-600 transition-colors"
                      >
                    ...
                    >
                      <p class="text-sm text-gray-500 mb-3">
                        Choisissez un type pour voir l'aperçu
                      </p>
                      <button
                        @click="$emit('create-qr-code', null)"
                        class="w-full bg-gray-300 text-gray-600 py-2.5 px-4 rounded-lg font-medium text-sm cursor-not-allowed"
                      >
    ...
    <div v-if="step === 2" class="max-w-lg mx-auto">
      <QRCodeForm
        ...
        @create-qr-code="$emit('create-qr-code', $event)"
      />
    </div>
  </section>
</template>
...
// Computed pour déterminer quel type afficher (hover prend la priorité)
const displayedType = computed(() => {
  return hoveredType.value || selectedType.value;
});
